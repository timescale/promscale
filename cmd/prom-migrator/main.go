...
func parseFlags(conf *config, args []string) {
	...
	flag.StringVar(&conf.promTSDBPath, "prom-tsdb-path", "", "Address for prometheus tsdb dir. This enables direct reading of blocks "+
		"instead of fetching data from /api/v1/read endpoint of a prometheus instance. Note: 'prom-tsdb-path' will replace the 'read-url'.")
