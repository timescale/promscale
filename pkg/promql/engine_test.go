...
func TestQueryTimeout(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(res.Err.Error(), e.Error()), "expected timeout error but got: %s", res.Err)
}
...
func TestEngineShutdown(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(res2.Err.Error(), e.Error()), "expected cancellation error but got: %s", res2.Err)
}
...
`)
	...
	for _, c := range cases {
		...
		require.Equal(t, c.Result.Value, res.Value)
	}
}
