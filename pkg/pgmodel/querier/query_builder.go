...
func buildPromQlFunctionCallAggregator(selectHints *storage.SelectHints, funcName string) (*aggregators, error) {
	...
	if selectHints.Step > 0 {
		...
		if resultStart != selectHints.End {
			return nil, fmt.Errorf("query start should equal query end")
		}
	}

	// On time ranges: given the parameters (scan_start, start, end), as
	// defined in buildSingleMetricSamplesQuery, the prom_* call that we
	// construct here has the rough structure of:
	// SELECT
	//   prom_*(scan_start, end, ...)
	// FROM ...
	// WHERE t >= scan_start AND t <= end
	//
...
func buildVectorSelectorFunctionCallAggregator(lookback int64, selectHints *storage.SelectHints, path []parser.Node) *aggregators {
	...
	// On time ranges: given the parameters (scan_start, start, end), as
	// defined in buildSingleMetricSamplesQuery, the vector_selector call that
	// we construct here has the rough structure of:
	// SELECT
	//   vector_selector(start, end, ...)
	// FROM ...
	// WHERE t >= scan_start AND t <= end
	//
	// Note: The actual WHERE clause parameters are set in
	...
	start := selectHints.Start + lookback
	end := selectHints.End
	qf := aggregators{
		valueClause: "vector_selector($%d, $%d, $%d, $%d, time, value)",
		valueParams: []interface{}{model.Time(start).Time(), model.Time(end).Time(), selectHints.Step, lookback},
		unOrdered:   true,
		tsSeries:    newRegularTimestampSeries(model.Time(start).Time(), model.Time(end).Time(), time.Duration(selectHints.Step)*time.Millisecond),
	}
	return &qf
}
