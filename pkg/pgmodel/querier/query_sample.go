...
func fetchSingleMetricSamples(ctx context.Context, tools *queryTools, metadata *evalMetadata) ([]sampleRow, parser.Node, error) {
	...
	rows, commit, err := pgxconn.QueryWithTimeoutFromCtx(ctx, tools.conn, query.sql, query.values...)
	if err != nil {
	...
	if err = commit(); err != nil {
		return nil, query.topNode, fmt.Errorf("query commit failed: %w", err)
	}
...
func fetchMultipleMetricsSamples(ctx context.Context, tools *queryTools, metadata *evalMetadata) ([]sampleRow, error) {
	...
	timeout, withTimeout := pgxconn.StatementTimeoutFromCtx(ctx)
	if withTimeout {
		pgxconn.QueueTimeout(batch, timeout)
	}
	...
	if withTimeout {
		if err = pgxconn.ExecBeginWithTimeout(batchResults); err != nil {
	...
	if withTimeout {
		if err = pgxconn.ExecCommit(batchResults); err != nil {
