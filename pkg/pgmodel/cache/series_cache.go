...
// TODO (james): DO NOT MERGE THIS. This map will never shrink!
var cacheKeyMap = make(map[string]*model.SeriesCacheKey)
var cacheKeyLock sync.RWMutex

func seriesCacheKey(b []byte) *model.SeriesCacheKey {
	cacheKeyLock.RLock()
	c, ok := cacheKeyMap[string(b)]
	cacheKeyLock.RUnlock()
	if ok {
		return c
	}
	// this allocs
	k := string(b)
	v := model.SeriesCacheKey(k)
	cacheKeyLock.Lock()
	cacheKeyMap[k] = &v
	cacheKeyLock.Unlock()
	return &v
...
func GenerateKey(labels []prompb.Label) (key *model.SeriesCacheKey, metricName string, error error) {
	...
	k := seriesCacheKey(builder.Bytes())

	return k, metricName, nil
}
...
func (t *StoredSeriesCacheImpl) PutSeries(key *model.SeriesCacheKey, series *model.StoredSeries) {
	t.cache.Insert(key, series, uint64(len(*key)+8))
}
