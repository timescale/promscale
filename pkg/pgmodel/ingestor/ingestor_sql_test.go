	...
	testCases := []struct {
		...
		},
		{
			...
			sqlQueries: []model.SqlQuery{
				...
				{
					Sql: "SELECT (_prom_catalog.get_or_create_series_id_for_label_array($1, l.elem)).series_id, l.nr FROM unnest($2::prom_api.label_array[]) WITH ORDINALITY l(elem, nr) ORDER BY l.elem",
					Args: []interface{}{
						"metric_1",
