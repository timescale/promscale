...
func TestLabelArrayCreator(t *testing.T) {
	...
	//setSeries := getSeries(t, unresolvedSeriesCache, labels.Labels{metricNameLabel, valTwo})
	//
	//series = []*model.Series{
	//	getSeries(t, unresolvedSeriesCache, labels.Labels{metricNameLabel, valOne}),
	//	setSeries,
	//}
	//labelMap = map[cache.LabelKey]cache.LabelInfo{
	//	makeLabelKey(metricNameLabel): cache.NewLabelInfo(100, 1),
	...
	//	makeLabelKey(valTwo):          cache.NewLabelInfo(2, 5),
	//}
	//res, ser, err = createLabelArrays(series, labelMap, 5)
	//require.NoError(t, err)
