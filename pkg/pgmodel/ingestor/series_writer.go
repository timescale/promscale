...
func NewSeriesWriter(conn pgxconn.PgxConn, labelsCache *cache.InvertedLabelsCache) *seriesWriter {
	return &seriesWriter{conn, labelsCache}
}
...
type perMetricInfo struct {
	...
	maxPos        int
	labelArraySet model.ArrayOfLabelArray
...
func (h *seriesWriter) fillLabelIDs(ctx context.Context, infos map[string]*perMetricInfo, labelMap map[cache.LabelKey]cache.LabelInfo) (model.SeriesEpoch, error) {
	...
	for metricName, info := range infos {
		...
		itemsPerBatch := 1000
		for i := 0; i < len(names); i += itemsPerBatch {
			high := i + itemsPerBatch
			if len(names) < high {
				high = len(names)
			}
			namesSlice := names[i:high]
			valuesSlice := values[i:high]
	...
	for _, info := range infoBatches {
		...
		err := func() error {
			var (
				pos         []int32
				labelIDs    []int32
				labelNames  pgtype.FlatArray[pgutf8str.Text]
				labelValues pgtype.FlatArray[pgutf8str.Text]
				names       []string
				values      []string
			)
			...
			names = pgutf8str.TextArrayToSlice(labelNames)
			values = pgutf8str.TextArrayToSlice(labelValues)
...
func (h *seriesWriter) buildLabelArrays(ctx context.Context, infos map[string]*perMetricInfo, labelMap map[cache.LabelKey]cache.LabelInfo) error {
	...
	for _, info := range infos {
		...
		info.labelArraySet = labelArraySet
	}

	return nil
}

// TODO test that for the items that are not in place there's no issue with
// Valid = false
func createLabelArrays(
	series []*model.Series,
	labelMap map[cache.LabelKey]cache.LabelInfo,
	maxPos int,
) (model.ArrayOfLabelArray, []*model.Series, error) {

	labelArraySet := make(model.ArrayOfLabelArray, 0, len(series))
	dest := 0
	for src := 0; src < len(series); src++ {
		...
		lArray := make(model.LabelArray, maxPos)
		maxIndex := 0
		for i := range names {
			...
			lArray[sliceIndex] = pgtype.Int4{
				Int32: int32(res.LabelID),
				Valid: true,
			}
