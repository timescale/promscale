...
func CheckDependencies(db *pgx.Conn, versionInfo VersionInfo, migrationFailedDueToLockError bool, allowPreRelease bool) (err error) {
	// TODO decide how to handle this after the switch
	if version.PromscaleVersion().LT(FirstAppVersionWithNewMigration()) {
		if err = CheckSchemaVersion(context.Background(), db, versionInfo, migrationFailedDueToLockError); err != nil {
			return err
		}
	}
	return extension.CheckVersions(db, migrationFailedDueToLockError, allowPreRelease)
}
