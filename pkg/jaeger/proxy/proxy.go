...
func (p *Proxy) Close(context.Context, *storage_v1.CloseWriterRequest) (*storage_v1.CloseWriterResponse, error) {
	err := p.conn.Close()
	if err != nil {
		return nil, fmt.Errorf("error closing connection to Promscale GRPC server: %w", err)
	}
	return nil, nil
}
