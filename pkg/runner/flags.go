...
func ParseFlags(cfg *Config, args []string) (*Config, error) {
	...
	fs.DurationVar(&cfg.ThroughputInterval, "tput-report", time.Second, "Setting duration to `0` will disable reporting throughput, else interval with unit at which throughput should be reported, e.g. `10s` or `3m.`")
