...
type IndexedCache struct {
	...
	storage  []element
	index    map[model.SeriesID]*indexElement
...
func (self *IndexedCache) applyPerfMetric(m *perfMetrics) {
	self.metrics = m
}
...
func (self *IndexedCache) Insert(key interface{}, value interface{}, sizeBytes uint64) (canonicalValue interface{}, in_cache bool) {
...
// starved
func (self *IndexedCache) InsertBatch(keys []interface{}, values []interface{}, sizesBytes []uint64) int {
...
func (self *IndexedCache) insert(key interface{}, value interface{}, size uint64) (existingElement *element, inserted bool, inCache bool) {
	...
	if len(self.storage) >= cap(self.storage) {
		...
		if insertLocation.value != nil {
			delete(self.index, insertLocation.value.(*model.StoredSeries).SeriesID())
		}
	...
	seriesId := value.(*model.StoredSeries).SeriesID()
	self.index[seriesId] = &indexElement{
		storageElement: insertLocation,
		lookupKey:      key,
	}
...
func (self *IndexedCache) Update(key, value interface{}, size uint64) (canonicalValue interface{}) {
	self.insertLock.Lock()
	defer self.insertLock.Unlock()
...
func (self *IndexedCache) EvictWithIndex(idxKey model.SeriesID) {
	self.insertLock.Lock()
	defer self.insertLock.Unlock()

	indexItem := self.index[idxKey]
	if indexItem == nil {
		return
	}
	self.evictions++
	...
	storageElement := indexItem.storageElement
	self.dataSize -= storageElement.size
	delete(self.elements, indexItem.lookupKey)
	delete(self.index, idxKey)
	*storageElement = element{}
}

func (self *IndexedCache) evict() (insertPtr *element) {
...
func (self *IndexedCache) GetValues(keys []interface{}, valuesOut []interface{}) (numFound int) {
	start := time.Now()
...
func (self *IndexedCache) Get(key interface{}) (interface{}, bool) {
	start := time.Now()
...
func (self *IndexedCache) Len() int {
	self.elementsLock.RLock()
	defer self.elementsLock.RUnlock()
	return len(self.storage)
}

func (self *IndexedCache) Evictions() uint64 {
	self.elementsLock.RLock()
	defer self.elementsLock.RUnlock()
	return self.evictions
}

func (self *IndexedCache) SizeBytes() uint64 {
	self.elementsLock.RLock()
	defer self.elementsLock.RUnlock()
...
func (self *IndexedCache) Cap() int {
	self.elementsLock.RLock()
	defer self.elementsLock.RUnlock()
