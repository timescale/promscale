...
func alterSearchPath(t *testing.T, db *pgxpool.Pool) {
	var username string
	...
	_, err = db.Exec(context.Background(), fmt.Sprintf("alter user %s set search_path to pg_temp", pgx.Identifier{username}.Sanitize()))
	if err != nil {
