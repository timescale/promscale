...
func TestSQLDropMetricChunk(t *testing.T) {
	...
	withDB(t, *testDatabase, func(db *pgxpool.Pool, t testing.TB) {
		...
		_, err = ingestor.Ingest(newWriteRequestWithTs(copyMetrics(resurrected)))
		if err == nil {
		...
		_, err = ingestor2.Ingest(newWriteRequestWithTs(copyMetrics(resurrected)))
		if err != nil {
			t.Error(err)
		}
...
func TestSQLDropAllMetricData(t *testing.T) {
	...
	withDB(t, *testDatabase, func(dbOwner *pgxpool.Pool, t testing.TB) {
		...
		defer ingestor2.Close()
		_, err = ingestor2.Ingest(newWriteRequestWithTs(copyMetrics(ts)))
		if err != nil {
			t.Fatal(err)
		}
