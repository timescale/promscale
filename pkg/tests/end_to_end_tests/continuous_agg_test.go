			...
			res: promql.Result{
				Value: promql.Matrix{
					promql.Series{
						...
						Points: []promql.Point{
							{T: 1577836800000, V: 952},
							{T: 1577840400000, V: 1912},
							{T: 1577844000000, V: 2872},
							{T: 1577847600000, V: 3832},
						},
					},
				},
			},
			...
			res: promql.Result{
				Value: promql.Matrix{
					promql.Series{
						...
						Points: []promql.Point{
							{T: 1577836800000, V: 0},
							{T: 1577840400000, V: 960},
							{T: 1577844000000, V: 1920},
							{T: 1577847600000, V: 2880},
						},
					},
				},
			},
			...
			res: promql.Result{
				Value: promql.Matrix{
					promql.Series{
						...
						Points: []promql.Point{
							{T: 1577836800000, V: 476},
							{T: 1577840400000, V: 1436},
							{T: 1577844000000, V: 2396},
							{T: 1577847600000, V: 3356},
						},
					},
				},
			},
...
WITH (timescaledb.continuous) AS
  SELECT public.time_bucket('1hour', time), series_id, max(value) as value, max(value) as max, min(value) as min, avg(value) as avg
    ...
    GROUP BY public.time_bucket('1hour', time), series_id`); err != nil {
		...
		if _, err := db.Exec(context.Background(), "SELECT prom_api.register_metric_view('cagg_schema', 'cagg', NULL)"); err != nil {
...
WITH (timescaledb.continuous) AS
  SELECT public.time_bucket('1hour', time), series_id, max(value) as value, max(value) as max, min(value) as min, avg(value) as avg
    ...
    GROUP BY public.time_bucket('1hour', time), series_id`)
		require.NoError(t, err)
		_, err = db.Exec(context.Background(), "SELECT prom_api.register_metric_view('cagg_schema', 'cagg', NULL)")
		require.NoError(t, err)
...
WITH (timescaledb.continuous) AS
  ...
  SELECT time, series_id, average(tw) as value
    FROM public.twa_cagg`); err != nil {
		...
		if _, err := db.Exec(context.Background(), "SELECT prom_api.register_metric_view('public', 'tw_1hour', NULL)"); err != nil {
			t.Fatalf("unexpected error while registering metric view: %s", err)
		}
