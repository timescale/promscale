...
func TestDeleteMetricSQLAPI(t *testing.T) {
	...
	withDB(t, *testDatabase, func(db *pgxpool.Pool, tb testing.TB) {
		...
		if _, err := ingestor.Ingest(ingstr.Request{Req: ingstr.NewWriteRequestWithTs(copyMetrics(ts))}); err != nil {
			t.Fatal(err)
		}
		...
		if _, err := ingestor.Ingest(ingstr.Request{Req: ingstr.NewWriteRequestWithTs(copyMetrics(tts))}); err != nil {
			t.Fatal(err)
		}
