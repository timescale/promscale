...
func ingestQueryTestDataset(db *pgxpool.Pool, t testing.TB, metrics []prompb.TimeSeries) {
	...
	cnt, _, err := ingestor.IngestMetrics(context.Background(), newWriteRequestWithTs(copyMetrics(metrics)))
...
func createMetricView(db *pgxpool.Pool, t testing.TB, schemaName, viewName, metricName string) {
	...
	if _, err := db.Exec(context.Background(), fmt.Sprintf("SELECT prom_api.register_metric_view('%s', '%s', NULL)", schemaName, viewName)); err != nil {
...
func TestMetricNameResolutionFromMultipleSchemas(t *testing.T) {
	withDB(t, *testDatabase, func(db *pgxpool.Pool, t testing.TB) {
		...
		_, _, err = ingestor.IngestMetrics(context.Background(), newWriteRequestWithTs([]prompb.TimeSeries{
			{
				Labels: []prompb.Label{
					{Name: pgmodel.MetricNameLabelName, Value: "metric_view"},
