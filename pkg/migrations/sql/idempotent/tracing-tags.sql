...
AS $func$
    SELECT coalesce
    (
        array_agg(jsonb_build_object(a.key_id, a.id))
        || array_agg(jsonb_build_array(jsonb_build_object(a.key_id, a.id))),
        array[]::jsonb[]
    )
...
RETURNS boolean
AS $func$
    SELECT (_tag_map).m @> ANY(SCHEMA_TRACING.eval_jsonb_path_exists(_op))
$func$
LANGUAGE SQL STABLE PARALLEL SAFE;
...
AS $func$
    SELECT array[
        jsonb_build_object(a.key_id, a.id),
        jsonb_build_array(jsonb_build_object(a.key_id, a.id))
    ]
    FROM SCHEMA_TRACING.tag a
    WHERE a.key = _op.tag_key
...
RETURNS boolean
AS $func$
    SELECT (_tag_map).m @> ANY(SCHEMA_TRACING.eval_equals(_op))
$func$
LANGUAGE SQL STABLE PARALLEL SAFE;
