...
SET search_path = pg_temp;
--redundant given schema settings but extra caution for security definers
REVOKE ALL ON FUNCTION _prom_catalog.get_new_pos_for_key(text, text[], boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION _prom_catalog.get_new_pos_for_key(text, text[], boolean) TO prom_reader; -- For exemplars querying.
GRANT EXECUTE ON FUNCTION _prom_catalog.get_new_pos_for_key(text, text[], boolean) TO prom_writer;
...
AS $$
    ...
    UNION ALL
    SELECT
        (_prom_catalog.get_new_pos_for_key(get_or_create_label_key_pos.metric_name, array[get_or_create_label_key_pos.key], false))[1]
    LIMIT 1
$$
LANGUAGE SQL VOLATILE;
...
CREATE OR REPLACE FUNCTION _prom_catalog.get_or_create_label_ids(metric_name TEXT, label_keys text[], label_values text[])
RETURNS TABLE(pos int[], id int[], label_key text[], label_value text[]) AS $$
        ...
        SELECT
           case when count(*) = count(known_pos) Then
              ...
              _prom_catalog.get_new_pos_for_key(get_or_create_label_ids.metric_name, array_agg(key_str), false)
           end as poss,
...
$$
LANGUAGE SQL VOLATILE;
COMMENT ON FUNCTION _prom_catalog.get_or_create_label_ids(text, text[], text[])
IS 'converts a metric name, array of keys, and array of values to a list of label ids';
GRANT EXECUTE ON FUNCTION _prom_catalog.get_or_create_label_ids(TEXT, text[], text[]) TO prom_writer;
...
CREATE OR REPLACE FUNCTION _prom_catalog.get_or_create_series_id_for_label_array(metric_name TEXT, larray prom_api.label_array, OUT table_name NAME, OUT series_id BIGINT)
AS $func$
DECLARE
  metric_id int;
BEGIN
...
$func$
LANGUAGE PLPGSQL VOLATILE;
GRANT EXECUTE ON FUNCTION _prom_catalog.get_or_create_series_id_for_label_array(TEXT, prom_api.label_array) TO prom_writer;
