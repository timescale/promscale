CALL SCHEMA_CATALOG.execute_everywhere('tracing_types', $ee$ DO $$ BEGIN
    ...
    CREATE TYPE SCHEMA_TRACING_PUBLIC.tag_map AS (m jsonb);
    GRANT USAGE ON TYPE SCHEMA_TRACING_PUBLIC.tag_map TO prom_reader;
...
CREATE INDEX ON SCHEMA_TRACING.span USING GIN (((tags).m) jsonb_path_ops); -- supports tag filters. faster ingest than json_ops
...
CREATE INDEX ON SCHEMA_TRACING.event USING GIN (((tags).m) jsonb_path_ops);
...
CREATE INDEX ON SCHEMA_TRACING.link USING GIN (((tags).m) jsonb_path_ops);
