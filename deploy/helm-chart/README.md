...
| `preserveConnectionSecret`      | If set to `false` allows to upgrade the connection secret when a `helm upgrade` for the helm release is performed. If set to `true` user and password of the connection secret are being preserved on a `helm upgrade`. | `true`                             |
